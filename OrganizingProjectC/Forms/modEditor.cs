...
namespace ModBuilder
{
    public partial class modEditor : Form
    {
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult res = MessageBox.Show("You have not entered a default value for the setting. Do you still want to add it?", "Adding setting", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            if (isEditing == false || editing == -1)
            {
                ...
                SQLiteCommand command = new SQLiteCommand("SELECT id, key, value FROM modsettings WHERE key = @key", conn);

                // To check we need just the key.
                command.Parameters.AddWithValue("@key", settingName.Text);

                // If it already exists, we got a result.
                SQLiteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                ...
                // Then lets insert the setting.
                command = new SQLiteCommand("INSERT INTO modsettings(key, value) VALUES(@key, @value)", conn);
            ...
            {
                // First lets check if the key doesn't already exist.
                SQLiteCommand command = new SQLiteCommand("SELECT id, key, value FROM modsettings WHERE key = @key AND id != @id", conn);

                // To check, we need the setting key and ID.
                command.Parameters.AddWithValue("@key", settingName.Text);
                ...
                // Create the query to update the setting.
                command = new SQLiteCommand("UPDATE modsettings SET key = @key, value = @value WHERE id = @id", conn);

                // Add in the parameters for this query.
                command.Parameters.AddWithValue("@id", editing);
                command.Parameters.AddWithValue("@key", settingName.Text);
                command.Parameters.AddWithValue("@value", settingValue.Text);
                ...
                // Done, so refresh the list of settings.
                refreshSettingTree();

                // And reset everything back to how it was.
                settingName.Text = "";
                settingValue.Text = "";
        ...
        private void refreshInstallationCodeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            customCodeInstall.Text = "";
            customCodeUninstall.Text = "";

            if (File.Exists(workingDirectory + "/Package/install.php"))
                customCodeInstall.Text = File.ReadAllText(workingDirectory + "/Package/install.php");
            if (File.Exists(workingDirectory + "/Package/uninstall.php"))
                customCodeUninstall.Text = File.ReadAllText(workingDirectory + "/Package/uninstall.php");
        }

        private string importFile(bool shouldbePHP = true)
        {
            OpenFileDialog fti = new OpenFileDialog();
            fti.CheckFileExists = true;
            fti.Title = "Select a file to import...";

            DialogResult dr = fti.ShowDialog();
            if (dr == DialogResult.OK && !String.IsNullOrEmpty(fti.FileName) && File.Exists(fti.FileName))
            {
                if (shouldbePHP && !fti.FileName.EndsWith(".php") && MessageBox.Show("The file you tried to import does not appear to be a PHP file. Are you still sure you wish to import it?", "Mod Builder", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
                        return null;

                try
                {
                    string import = File.ReadAllText(fti.FileName);

                    if (String.IsNullOrWhiteSpace(import))
                    {
                        MessageBox.Show("Nothing to import.", "Mod Builder", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return null;
                    }

                    if (shouldbePHP && !import.StartsWith("<?") && MessageBox.Show("The file you tried to import does not (immediately) contain PHP start tags. Are you still sure you wish to import it?", "Mod Builder", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
                            return null;
        ...
        private void importCustomCodeInstall_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            string import = importFile();

            if (import != null)
                customCodeInstall.Text = import;
        }

        private void importCustomCodeUninstall_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            string import = importFile();

            if (import != null)
                customCodeUninstall.Text = import;
        }

        private void importInstallDatabaseCode_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            string import = importFile();

            if (import != null)
                installDatabaseCode.Text = import;
        }

        private void importUninstallDatabaseCode_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            string import = importFile();

            if (import != null)
                uninstallDatabaseCode.Text = import;
        }
